...
class MotionHeatmap:
    def __init__(
        ...
        selected_columns=["frame_number", "x_min", "y_min", "x_max", "y_max"]
        self.detections[selected_columns] = self.detections[selected_columns].apply(lambda x: pd.to_numeric(x, downcast='unsigned'))  # FIXME detections should be saved as integers from the very beginning
    ...
    def __call__(
        ...
        while self.video_reader.more():
            ...
            processed_image = self.process(frame, corresponding_detections, frame_count)
            frame_count += 1
...
class GPUMotionHeatmap(MotionHeatmap):
    ...
    def setup(
        ...
        try:
            video_reader = VideoReader(
                video_file=source_path, backend="gpu", **self.video_reader_kwargs
            ).start()
        except FileNotFoundError:
            self.logger.warning(
                f"Video file not found. Please check the file at {source_path}"
        ...
        self.cumulative_image = cv2.cuda_GpuMat(
